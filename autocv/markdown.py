"""
create and render CV in Markdown format

NOTE: only render_publications has been ported so far...
...
class MarkdownCV:
    ...
    def __init__(self, researcher, font='TeX Gyre Termes', etalthresh=10, etalnum=3):
        ...
        self.sections_to_write = ['publications']
    ...
    def write_md(self, outfile='autocv.md'):
    ...
    def render_publications(self):
        ...
        for year in years:
            ...
            for pub in keys_sorted_by_author:
                ...
                line = self.researcher.publications[pub].format_reference(self.etalthresh, self.etalnum, 'md' )

                if hasattr(self.researcher.publications[pub], 'PMC') and self.researcher.publications[pub].PMC is not None:
                    line += ' [OA](https://www.ncbi.nlm.nih.gov/pmc/articles/%s)' % self.researcher.publications[pub].PMC

                for linktype in links:
                    if pub in links[linktype]:
                        line += ' [%s](%s)' % (
                            linktype, links[linktype][pub])
                ...
                if self.researcher.publications[pub].type not in ['book', 'monograph'] and hasattr(self.researcher.publications[pub], 'DOI'):
                    line += ' [DOI](http://dx.doi.org/%s)' % self.researcher.publications[pub].DOI
